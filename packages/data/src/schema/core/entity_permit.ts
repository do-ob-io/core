...
export const table = pgTable('permit', {
  ...
  ambit: smallint('ambit').notNull().default(0), // The ambits restricts the action to be performed within certain scopes.
  rate: smallint('rate').notNull().default(0), // The rate level at which the permit allows the action to be performed.
}, (table) => ({
